...
PARAMS = {
	"latitude": 51.9411,
	"longitude": -1.5453,
	"daily": ["sunrise", "sunset", "temperature_2m_mean", "cloud_cover_mean", "leaf_wetness_probability_mean", "precipitation_probability_mean", "relative_humidity_2m_mean", "visibility_mean", "wind_speed_10m_mean", "wind_gusts_10m_mean", "wind_direction_10m_dominant", "uv_index_max", "daylight_duration", "sunshine_duration", "rain_sum", "showers_sum", "snowfall_sum", "precipitation_sum", "precipitation_hours"],
	"timezone": "GMT",
	"forecast_days": 15,
}
def extract_cforecast():
    params=PARAMS.copy()
    ...
    daily_leaf_wetness_probability_mean = daily.Variables(4).ValuesAsNumpy()
    daily_precipitation_probability_mean = daily.Variables(5).ValuesAsNumpy()
    ...
    daily_uv_index_max = daily.Variables(11).ValuesAsNumpy()
    ...
    cforecast_df = pd.DataFrame(data = daily_data)
    cforecast_df["load_date"] = pd.Timestamp.now()
    return cforecast_df